---
title: SRE Basics - Securing Postgres and Finalizing Observability (Part 7)
...
# SRE Basics - Securing Postgres and Finalizing Observability (Part 7)
...
```
_`kubectl get secret postgres-secret` output_
...
```
_Postgres logs showing clean startup_
...
```
_Flask logs showing DB connection success_

## Step 4: Verify USE and RED Metrics

With the database issues resolved, I checked `<flask-ip>:8000` and saw the metrics I needed: `request_count`, `request_processing_seconds`, `error_count`, `cpu_usage_percent`, and `memory_usage_bytes`. This covers USE (Utilization: CPU and memory; Saturation: request time; Errors: service-level) and RED (Rate, Errors, Duration) for Prometheus to scrape.
...
> Test metrics with `curl` before assuming Prometheus can see them, as port mismatches can happen.
{: .prompt-warning }
...
```
_`curl <flask-ip>:8000` showing USE and RED metrics_
